<html>
    ...
    <body>
        <h1 style="text-align: center;color:black;">ENTER THE MANAGER DETAILS</h1>
...
<div class="form-group">
  ...
  <div class="input-group">
  <span class="input-group-addon"><i class="glyphicon glyphicon-user"></i></span>
  <input type ="date" name="date"  class="form-control" placeholder="Enter the join date " >
    </div>
  </div>
</div>
                ...
                if(!empty($_POST['dob']))
                {
					...
					mysqli_select_db($conn,"car");
						...
						{	
   						 ...
   						 while($row = $result->fetch_assoc())
						 {
							$mid=$row['max(manager_id)']+1;
		
						}
						}	
						$sql=mysqli_query($conn,"INSERT INTO manager 										values('$mid','$_POST[fname]','$_POST[lname]','$_POST[dob]','$_POST[date]','$_POST[cid]','$password')");
						$sql1=mysqli_query($conn,"INSERT INTO manager_pro 										values('$mid','$_POST[phno]','$_POST[email]','$_POST[sal]','$_POST[addr]')");
...
<div class="form-group">
  <label class="col-md-4 control-label"></label>
  <div class="col-md-4"><br>
      <button type="submit" name="submit" class="btn btn-primary btn-lg" ><a href="adminview.php" style="text-decoration: none;color:white;">Back</a><span class="glyphicon glyphicon-send"></span></button>
  </div>
</div>
